   ...
   def inference(self, x_all, subgraph_loader, device):
       ...
       for i, conv in enumerate(self.convs):
           ...
           for batch_size, n_id, adj in subgraph_loader:
               ...
               # if i == 0:
               #     x2 = x_all2[n_id].to(device)
               #     x = x + F.tanh(self.lin(x2))
               x_target = x[:size[1]]
...
def train(model, loader, optimizer, device, alpha):
   ...
   for data in loader:
       ...
       optimizer.zero_grad()
       #x = F.dropout(x, p=self.dropout, training=self.training)
       out = model( data.x1, data.edge_index)
       out2 = model(data.x2, data.edge_index)
       y = data.y.squeeze(1)
       loss = alpha * F.nll_loss(out[data.train_mask], y[data.train_mask]) +  (1-alpha) * F.nll_loss(out2[data.train_mask], y[data.train_mask])
       loss.backward()
       optimizer.step()
...
def main():
   ...
   for run in range(args.runs):
       ...
       for epoch in range(1, 1 + args.epochs):
           loss = train(model, loader, optimizer, device, alpha)

...
def get_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=50,
        help="Parameter balances loss from hard labels and teacher outputs",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0.2,
        help="Parameter balances loss from text-only student and GNN teacher, take values in [0, 1]",
    )
...
def run(args):
    ...
    g, labels, known_labels, mask, idx_train, idx_val, idx_test = load_data(
        args.dataset,
        args.data_path,
